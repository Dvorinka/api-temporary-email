...
git clone https://github.com/your-username/api-temporary-email.git
cd api-temporary-email
...
- **Base URL**: http://localhost:8080/v1/temporary/
...
```bash
curl -H "Authorization: Bearer dev-temporary-key" \
     http://localhost:8080/v1/temporary/endpoint
```
...
curl -H "X-RapidAPI-Proxy-Secret: your-secret" \
     ...
     https://your-api.p.rapidapi.com/v1/temporary/endpoint
```
...
```bash
# Build image
docker build -t api-temporary-email .

# Run container
docker run -p 8080:8080 \
  -e TEMPORARY_EMAIL_API_KEY=your-key \
...
2. Repository: `your-username/api-temporary-email`
...
| `TEMPORARY_EMAIL_API_KEY` | Service API key | dev-temporary-key |
...
TEMPORARY_EMAIL_API_KEY=your-development-key
```
...
TEMPORARY_EMAIL_API_KEY=your-production-api-key
...
- Domain age analysis
- Known spam provider identification
- Risk scoring
...
2. **Docker Build Issues**
   ...
   docker build --no-cache -t api-temporary-email .
   ```

3. **Environment Issues**
   ```bash
   # Check environment variables
   docker-compose logs temporary-email-api
   ```
...
### Base URL
```
http://localhost:8080/v1/temporary/
```
